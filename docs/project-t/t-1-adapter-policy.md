...
- `verified`
- `proof_text`
- VRChat producer 中觀察到的 compact `v`（語意與 consumer dependency 待驗證）
...
| accepted mapping | true | Phase 0 證實後測試候選 mapping |
| non-accepted mapping | false | Phase 0 證實後測試候選 mapping |
...
在此之前，`verified` 與 `proof_text` 是網站正式相容 contract；`v` 是凍結的 VRChat producer field，須待 actual consumer inventory 證實語意與依賴，三者都不可任意清理。
