# VRChat Racing Hub T0 參考規格草案

**文件代號：** D4  
**版本：** Draft 0.1 / Goal Aligned  
**狀態：** Non-normative Reference Draft / 非規範性參考文件  
**日期：** 2026-07-16  
...
> 本文件只保存 T0 多來源架構的設計參考，不是目前 Project T 的主要產品決策、治理 SSOT、production contract 或 implementation gate。

發生衝突時，依下列順序判定：

1. Owner 已確認的最新決策與 `docs/SSOT.md`。
2. `docs/project-t/t-1-product-structure.md` 的 T-1 產品定位。
3. `t-1-current-schema-map.md`、contract inventory 與實際 pipeline／consumer 證據。
4. 本參考草案。

不得僅引用本文件宣稱 T0 已核准、T-1 必須採用多來源架構，或某項 production 行為已完成。

## 已知實作註記（2026-07-21）

- Route definition／geometry JSON 目前由人工繪製；它與 canonical database 生成的 leaderboard route JSON 是不同 artifact，不應共用 authority。
- Unity／Udon 模組目前存在於另一台電腦本機；在版本、來源路徑與可重現接入測試進入正式紀錄前，只能標示為「已有本地實作」。
- Phase 0 已由 generator 與實際 Udon consumer 證實 compact `v = int(verified)`，consumer 以 `v == 1` 顯示驗證標記。其餘 payload 大小、快取、timeout、offline 與 schema mismatch 行為仍需個別驗證。
...
Phase 0 已證實 compact `v = int(verified)` 的 producer／consumer 語意。下列其餘事項仍需直接測試或版本化證據：
